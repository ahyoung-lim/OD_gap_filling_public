"""03_down_DM_full_repeatedCV_v2.py

Re-run of the DM repeated 3-fold CV (DM_v3_pooled_jax_v3) with three changes
vs the original January 2026 run:

  (a) 4 chains x 1000 draws (was 2 x 500) for stronger MCMC diagnostics.
  (b) Row-level posterior predictive output (adm_0_name, Year, month, observed
      and predicted incidence, quantiles, CRPS) for region-stratified tables.
  (c) Full-parameter convergence diagnostics CSV (R-hat / ESS for every latent
      variable, not just the 5 key scalars).

Outputs (the January 2026 results under runs/CV/20260126/ are left untouched;
masks are read from there):
  runs/CV/20260126/dm_rerun_4chains/
    down_DM_full_CV_rep01/
      pymc_metrics_monthly_fold.csv            (fold-level metrics)
      pymc_convergence_diagnostics_fold.csv    (summary diagnostics, key scalars)
      pymc_convergence_by_variable_fold.csv    (per-variable R-hat max / ESS min, all variables)
      pymc_convergence_p_missing_fold.csv      (per-element R-hat / ESS for p_missing)
      pymc_hyperparam_summaries_fold.csv
      pymc_row_predictions_fold.csv            (row-level predictions)
    ...rep02, rep03...
    pymc_metrics_monthly_repeatedCV_fold.csv
    pymc_metrics_monthly_repeatedCV_overall.csv
    pymc_convergence_diagnostics_fold.csv
    pymc_convergence_by_variable_fold.csv
    pymc_convergence_p_missing_fold.csv
    pymc_hyperparam_summaries_fold.csv
    pymc_row_predictions_fold.csv
    down_DM_full_prior_spec.csv
"""
...
# Outputs go to a separate folder so the January 2026 CV results are untouched;
# masks are read from the original CV folder.
BASE_DIR = os.path.join("runs", "CV", "20260126", "dm_rerun_4chains")
MASK_ROOT = os.path.join("runs", "CV", "20260126", "masks")
...
# Sampler settings — v2: 4 chains x 1000 draws (reviewer request)
CHAINS = 4
DRAWS = 1000
TUNE = 1500
TARGET_ACCEPT = 0.99
...
def _collapse_a_draws(trace: az.InferenceData, a_name: str = "a_missing", max_draws: int | None = None) -> np.ndarray:
    ...
    if max_draws is not None and a.shape[0] > max_draws:
        # Evenly spaced subsample across the stacked (chain, draw) axis so that
        # every chain contributes to the posterior predictive draws.
        idx = np.linspace(0, a.shape[0] - 1, max_draws).round().astype(int)
        a = a[idx]
    return a
...
def score_fold_incidence_rowlevel(
    *,
    predictive_samples_counts: np.ndarray,
    truth_counts: np.ndarray,
    pop_month: np.ndarray,
    test_idx: List[Tuple[str, int]],
) -> pd.DataFrame:
    """Row-level predictions: one row per (adm_0_name, Year, month).

    Uses the same row filter (finite truth, pop >= POP_FLOOR) and the same
    minimum-draw threshold as score_fold_incidence, so that aggregating these
    rows reproduces the fold-level metrics.
    """
    pop_month = np.asarray(pop_month, float)
    truth_counts = np.asarray(truth_counts, float)

    N_cy, M = truth_counts.shape
    S = predictive_samples_counts.shape[0]
    thr = max(MIN_DRAWS, int(np.ceil(MIN_PROP * S)))
    ...
    rows = []
    for i, (country, year) in enumerate(test_idx):
        for m in range(M):
            pop_val = pop_month[i, m]
            obs_c = truth_counts[i, m]
            obs_inc = inc_truth[i, m]
            draws = inc_pred[:, i, m]
            draws = draws[np.isfinite(draws)]

            if len(draws) < thr or not np.isfinite(obs_inc) or not np.isfinite(pop_val) or pop_val < POP_FLOOR:
                rows.append(dict(
                    adm_0_name=country, Year=int(year), month=m + 1,
                    obs_count=float(obs_c), obs_inc=float(obs_inc),
                    pred_mean_inc=np.nan, pred_median_inc=np.nan,
                    q10_inc=np.nan, q90_inc=np.nan, crps=np.nan,
                ))
                continue

            pred_mean = float(np.nanmean(draws))
            pred_median = float(np.nanmedian(draws))
            q10 = float(np.nanquantile(draws, 0.10))
            q90 = float(np.nanquantile(draws, 0.90))

            y = float(obs_inc)
            s = np.sort(draws)
            S0 = len(s)
            a_val = float(np.mean(np.abs(s - y)))
            k = np.arange(1, S0 + 1, dtype=float)
            b_val = float((2.0 / (S0**2)) * np.sum((2.0 * k - S0 - 1.0) * s))
            crps_val = a_val - 0.5 * b_val

            rows.append(dict(
                adm_0_name=country, Year=int(year), month=m + 1,
                obs_count=float(obs_c), obs_inc=float(obs_inc),
                pred_mean_inc=pred_mean, pred_median_inc=pred_median,
                q10_inc=q10, q90_inc=q90, crps=crps_val,
            ))

    return pd.DataFrame(rows)


def convergence_diagnostics_all_variables(trace: az.InferenceData) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """R-hat / ESS for every variable in the posterior (free and deterministic).

    Uses az.rhat / az.ess directly (no HDI / MCSE), which is much cheaper than
    az.summary on the large deterministic arrays.

    Returns two frames:
      by_variable: one row per variable — number of elements, R-hat max,
                   ESS bulk/tail min, and the count of elements with R-hat > 1.01.
      p_missing:   one row per element of p_missing (obs_test x month), the
                   quantity that feeds the posterior predictive scoring.
    """
    rhat = az.rhat(trace)
    ...
    for v in rhat.data_vars:
        r = np.asarray(rhat[v].values, float).ravel()
        eb = np.asarray(ess_b[v].values, float).ravel()
        et = np.asarray(ess_t[v].values, float).ravel()
        by_var.append(dict(
            variable=v,
            n_elements=int(r.size),
            rhat_max=float(np.nanmax(r)) if r.size else np.nan,
            rhat_n_gt_1p01=int(np.nansum(r > 1.01)),
            rhat_n_gt_1p05=int(np.nansum(r > 1.05)),
            ess_bulk_min=float(np.nanmin(eb)) if eb.size else np.nan,
            ess_tail_min=float(np.nanmin(et)) if et.size else np.nan,
        ))
    by_var_df = pd.DataFrame(by_var)

    pm_df = pd.DataFrame()
    if "p_missing" in rhat.data_vars:
        r = rhat["p_missing"]
        dims = list(r.dims)
        d_obs, d_mon = dims[0], dims[1]
        obs_labels = np.asarray(r[d_obs].values)
        mon_labels = np.asarray(r[d_mon].values)
        rv = np.asarray(r.values, float)
        ebv = np.asarray(ess_b["p_missing"].values, float)
        etv = np.asarray(ess_t["p_missing"].values, float)
        oo, mm = np.meshgrid(np.arange(rv.shape[0]), np.arange(rv.shape[1]), indexing="ij")
        pm_df = pd.DataFrame(dict(
            obs_test=obs_labels[oo.ravel()],
            month=mon_labels[mm.ravel()],
            r_hat=rv.ravel(),
            ess_bulk=ebv.ravel(),
            ess_tail=etv.ravel(),
        ))

    return by_var_df, pm_df
...
def fit_score_one(
    *,
    ...
    priors: Optional[Dict[str, float]] = None,
) -> Tuple[Dict[str, object], Dict[str, object], List[Dict[str, object]], pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Fit the model for one (rep, mask_type, fold) and return:
      - metrics row
      - convergence row (key scalars)
      - hyperparam rows
      - row-level predictions DataFrame
      - per-variable convergence DataFrame (all variables)
      - per-element convergence DataFrame for p_missing
    """
    ...
    if n_test_years == 0:
        ...
        return metrics, conv, [], pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    # Full-parameter convergence diagnostics (all latent variables)
    conv_byvar_df, conv_pmis_df = convergence_diagnostics_all_variables(trace)
    for _df in (conv_byvar_df, conv_pmis_df):
        if not _df.empty:
            _df.insert(0, "model", model_name)
            _df.insert(1, "mask_type", mask_type)
            _df.insert(2, "fold", fold)
            _df.insert(3, "rep", rep_i)
    if not conv_pmis_df.empty:
        # obs_test is a positional index into the test country-years; label it
        _lab = pd.DataFrame(A["test_idx"], columns=["adm_0_name", "Year"])
        conv_pmis_df["adm_0_name"] = _lab["adm_0_name"].values[conv_pmis_df["obs_test"].astype(int).values]
        conv_pmis_df["Year"] = _lab["Year"].values[conv_pmis_df["obs_test"].astype(int).values]
        conv_pmis_df["month"] = conv_pmis_df["month"].astype(int) + 1
    ...
    # Row-level predictions for regional metrics
    row_pred_df = score_fold_incidence_rowlevel(
        predictive_samples_counts=predictive_counts,
        truth_counts=A["test_counts"],
        pop_month=A["test_pop_month"],
        test_idx=A["test_idx"],
    )
    row_pred_df.insert(0, "model", model_name)
    row_pred_df.insert(1, "mask_type", mask_type)
    row_pred_df.insert(2, "fold", fold)
    row_pred_df.insert(3, "rep", rep_i)
    ...
    return metrics, conv, hyp_rows, row_pred_df, conv_byvar_df, conv_pmis_df
...
def main() -> None:
    ...
    all_row_pred: List[pd.DataFrame] = []
    all_conv_byvar: List[pd.DataFrame] = []
    all_conv_pmis: List[pd.DataFrame] = []
    ...
    for rep_i, mask_seed in enumerate(MASK_SEEDS, start=1):
        ...
        rowpred_fp  = os.path.join(rep_dir, "pymc_row_predictions_fold.csv")
        convbyvar_fp = os.path.join(rep_dir, "pymc_convergence_by_variable_fold.csv")
        convpmis_fp  = os.path.join(rep_dir, "pymc_convergence_p_missing_fold.csv")
        ...
        for mask_type, mask_path in masks.items():
            ...
            for fold in FOLDS:
                ...
                m, c, h, rp, cbv, cpm = fit_score_one(
                    df_long=df_long,
                    rep_i=rep_i,
                ...
                if not rp.empty:
                    append_list_csv(rowpred_fp, rp.to_dict("records"))
                if not cbv.empty:
                    append_list_csv(convbyvar_fp, cbv.to_dict("records"))
                if not cpm.empty:
                    append_list_csv(convpmis_fp, cpm.to_dict("records"))
                done.add(job)
                ...
                if (
                    ...
                    m2, c2, h2, _rp2, _cbv2, _cpm2 = fit_score_one(
                        df_long=df_long,
                        rep_i=rep_i,
                        mask_seed=mask_seed_used,
                        mask_type=mask_type,
        ...
        if os.path.exists(rowpred_fp):
            all_row_pred.append(pd.read_csv(rowpred_fp))
        if os.path.exists(convbyvar_fp):
            all_conv_byvar.append(pd.read_csv(convbyvar_fp))
        if os.path.exists(convpmis_fp):
            all_conv_pmis.append(pd.read_csv(convpmis_fp))
    ...
    rowpred_all = pd.concat(all_row_pred, ignore_index=True) if all_row_pred else pd.DataFrame()
    convbyvar_all = pd.concat(all_conv_byvar, ignore_index=True) if all_conv_byvar else pd.DataFrame()
    convpmis_all = pd.concat(all_conv_pmis, ignore_index=True) if all_conv_pmis else pd.DataFrame()
    ...
    rowpred_all.to_csv(os.path.join(BASE_DIR, "pymc_row_predictions_fold.csv"), index=False)
    convbyvar_all.to_csv(os.path.join(BASE_DIR, "pymc_convergence_by_variable_fold.csv"), index=False)
    convpmis_all.to_csv(os.path.join(BASE_DIR, "pymc_convergence_p_missing_fold.csv"), index=False)
