# CV_tab1_final_models_performance.R
#
# Supplementary tables of repeated cross-validation performance of the final
# gap-filling models (weekly and monthly imputation: GAM, INLA; annual-to-monthly
# disaggregation: DM, INLA unscaled, INLA scaled).
#
# Same structure as script/arx/formatting_table.R but adds:
...
# DM fold-level metrics come from the CV re-run with 4 chains x 1000 draws
# (script/CV/03_down_DM_full_repeatedCV_v2.py), stored in a sub-folder.
dm_fold_file <- function(run_dir) {
  file.path(run_dir, "dm_rerun_4chains", "pymc_metrics_monthly_repeatedCV_fold.csv")
}

read_dm_fold <- function(run_dir) {
  fp <- dm_fold_file(run_dir)
...
# --- Tables in the layout of the supplementary document -----------------------
# The wide tables above (all predictor variants) stay in runs/CV as the
# intermediate record; these files carry exactly the columns and rounding of
# the supplementary tables (MAE / RMSE of the median predictor, normalised by
# the mean observed incidence; 80% coverage; CRPS).
docx_dir <- file.path("output", "tables")
dir.create(docx_dir, recursive = TRUE, showWarnings = FALSE)
task_labels <- c(interp = "Interpolation",
                 extrap_past = "Extrapolation (backcast)",
                 extrap_future = "Extrapolation (forecast)")
docx_table <- function(tbl) {
  # the imputation tables carry pre-formatted strings, the disaggregation table
  # numerics: coerce before formatting
  f <- function(x, fmt) sprintf(fmt, as.numeric(x))
  tbl %>%
    mutate(mask_type = trimws(mask_type)) %>%
    transmute(
      Task = unname(task_labels[mask_type]), Model, N = as.integer(n_test),
      `Mean observed incidence` = f(obs_mean_inc, "%.1f"),
      MAE = f(MAE_median, "%.1f"), RMSE = f(RMSE_median, "%.1f"),
      nMAE = f(nMAE_median, "%.2f"), nRMSE = f(nRMSE_median, "%.1f"),
      `80% coverage` = f(cov80, "%.2f"), CRPS = f(crps, "%.1f")
    )
}
for (x in list(list(tbl_weekly, "weekly"), list(tbl_monthly, "monthly"), list(tbl_dm, "disaggregation"))) {
  fp <- file.path(docx_dir, sprintf("CV_tab1_cv_performance_%s.csv", x[[2]]))
  write_csv(docx_table(x[[1]]), fp)
  cat("wrote", fp, "\n")
}
