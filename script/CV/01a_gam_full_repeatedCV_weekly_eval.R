...
formulas <- list(
  gam_bench_org = gam_w_bench_original
)
...
find_inla_mask_dir <- function(base_dir, rep_i) {
  ...
  candidates <- c(
    file.path(base_dir, "masks", paste0("weekly_", rep_tag)), # e.g. masks/weekly_rep01
    file.path(base_dir, "masks", paste0("weekly_rep", sprintf("%02d", rep_i))), # e.g. masks/weekly_rep01
    file.path(base_dir, "masks", rep_tag), # e.g. masks/rep01
    file.path(base_dir, "masks") # fallback (single-run masks)
  )
  for (d in candidates) {
    if (file.exists(file.path(d, "inla_full_mask_w_interp.rds"))) {
      return(d)
    }
  }
  stop(
...
for (rep_i in reps) {
  ...
  rep_tag <- sprintf("rep%02d", rep_i)
  out_dir_rep <- file.path(base_dir, sprintf("gam_full_CV_weekly_%s", rep_tag)) # <- your requested naming
  dir.create(out_dir_rep, recursive = TRUE, showWarnings = FALSE)

  inla_mask_dir_rep <- find_inla_mask_dir(base_dir, rep_i)

  mask_files <- list(
    interp        = file.path(inla_mask_dir_rep, "inla_full_mask_w_interp.rds"),
    extrap_future = file.path(inla_mask_dir_rep, "inla_full_mask_w_extrap_future.rds"),
    extrap_past   = file.path(inla_mask_dir_rep, "inla_full_mask_w_extrap_past.rds")
  )
  ...
  write.csv(consolidated_overall,
    file.path(out_dir_rep, "gam_metrics_weekly.csv"),
    row.names = FALSE
  )
  write.csv(consolidated_fold,
    file.path(out_dir_rep, "gam_metrics_weekly_fold.csv"),
    row.names = FALSE
  )
...
write.csv(overall_allreps,
  file.path(base_dir, "gam_metrics_weekly_repeatedCV_overall.csv"),
  row.names = FALSE
)
write.csv(fold_allreps,
  file.path(base_dir, "gam_metrics_weekly_repeatedCV_fold.csv"),
  row.names = FALSE
)
...
write.csv(summary_overall,
  file.path(base_dir, "gam_metrics_weekly_repeatedCV_summary.csv"),
  row.names = FALSE
)
